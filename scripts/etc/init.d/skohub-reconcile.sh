...
# Default-Stop:      0 1 6
# Short-Description: skohub-reconcile as a node server
# Description:       skohub-reconcile as a node server available over the
#                    network.
### END INIT INFO

# this file should be placed into /etc/init.d/
### if you copy it to init.d:
# set the home directory to your skohub-reconcile installation as an absolute path:
HOME_=$HOME
### END copy to init.d

PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin
HOME_SKOHUB=$HOME_/git/skohub-reconcile/scripts/
DAEMON_START_SCRIPT=$HOME_SKOHUB/start.sh
NAME=skohub-reconcile
PID_FILE=$HOME_SKOHUB/$NAME.pid
DESC="skohub-reconcile as a node server"
RUN_AS_USER=lod
